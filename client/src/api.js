...
export const api = {
  ...
  folderListings: (params) => request("GET", "/api/folder-listings", { params }),
  folderListing: (rootId, path) => request("GET", `/api/folder-listings/${rootId}/${encodeURIComponent(path || ".")}`),
  saveFolderListing: (rootId, path, body) => request("PUT", `/api/folder-listings/${rootId}/${encodeURIComponent(path || ".")}`, { body }),
  checkFolderListing: (rootId, path) => request("POST", `/api/folder-listings/${rootId}/${encodeURIComponent(path || ".")}/check`),
  checkAllFolderListings: (rootId) => request("POST", "/api/folder-listings/check-all", { params: { root_id: rootId } }),
  draftFolderListings: (body) => request("POST", "/api/folder-listings/draft", { body }),
  draftProgress: () => request("GET", "/api/folder-listings/draft/progress"),
  exportFolderListings: (rootId, format, status) => request("GET", "/api/folder-listings/export", { params: { root_id: rootId, format, status } }),
